...
def _service_fallback(key: str, message: str):
    return {
        "success": True,
        key: [],
    ...
    try:
        ...
        if _is_database_unavailable(e):
            return _service_fallback(
                "doctors",
                "Doctor directory is temporarily unavailable. Please try again in a few minutes.",
            )
...
async def get_doctor_profile(wallet: str):
    ...
    try:
        ...
        if _is_database_unavailable(e):
            return _service_fallback(
                "profiles",
                "Doctor profile service is temporarily unavailable. Please try again in a few minutes.",
            )
...
async def get_doctor_grants(wallet: str):
    ...
    try:
        ...
        if _is_database_unavailable(e):
            return _service_fallback(
                "grants",
                "Access grants are temporarily unavailable. Please try again in a few minutes.",
            )
...
async def get_doctor_appointments(wallet: str):
    ...
    try:
        ...
        if _is_database_unavailable(e):
            return _service_fallback(
                "appointments",
                "Appointment service is temporarily unavailable. Please try again in a few minutes.",
            )
...
async def get_consultation_notes(doctor_wallet: str, patient_wallet: str):
    try:
        ...
        if _is_database_unavailable(e):
            return _service_fallback(
                "notes",
                "Consultation notes are temporarily unavailable. Please try again in a few minutes.",
            )
