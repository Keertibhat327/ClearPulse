...
async def get_records(wallet: str):
    ...
    try:
        ...
        if e.response.status_code == 404:
            ...
            print(f"InsForge service temporarily unavailable for records: {str(e)}")
            return {
                "success": True, 
                "records": [],
                "service_status": "temporarily_unavailable",
                "message": "Records service is temporarily unavailable. Please try again in a few minutes."
            }
        raise HTTPException(status_code=500, detail=f"Failed to fetch records: {str(e)}")
    except Exception as e:
        if _is_database_unavailable(e):
            print(f"InsForge service unavailable for records: {str(e)}")
            return {
                "success": True, 
                "records": [],
                "service_status": "temporarily_unavailable",
    ...
    try:
        ...
        if _is_database_unavailable(e):
            return {
                ...
                "grants": [],
                "service_status": "temporarily_unavailable",
                "message": "Access grants are temporarily unavailable. Please try again in a few minutes.",
            }
