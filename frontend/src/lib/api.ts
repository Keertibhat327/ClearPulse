...
export interface ServiceFallback {
    service_status?: "available" | "temporarily_unavailable";
    message?: string;
}
...
async function get<T>(path: string, retries: number = 1): Promise<T> {
    let res;
    let lastError: Error;
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            if (res.status === 503 && attempt < retries) {
                console.warn(`Attempt ${attempt + 1} failed with 503, retrying in 1 second...`);
            ...
            lastError = new Error(`Connection failed: Could not reach backend at ${API_URL}`);
            if (attempt < retries) {
                console.warn(`Attempt ${attempt + 1} failed, retrying in 1 second...`);
                await new Promise(resolve => setTimeout(resolve, 1000));
                continue;
            }
        }
...
export const getPatientRecords = (wallet: string) =>
    get<{ success: boolean; records: any[] } & ServiceFallback>(`/api/records/${encodeURIComponent(wallet)}`);
...
export const getAppointments = (wallet: string) =>
    get<{ success: boolean; appointments: any[] } & ServiceFallback>(`/api/appointments/${encodeURIComponent(wallet)}`);
...
export const getAllDoctors = (specialty?: string) => {
    ...
    return get<{ success: boolean; doctors: any[] } & ServiceFallback>(`/api/doctors${query}`);
}

export const getDoctorProfile = (wallet: string) =>
    get<{ success: boolean; profiles: any[] } & ServiceFallback>(`/api/doctor/profile/${encodeURIComponent(wallet)}`);
...
export const getDoctorGrants = (wallet: string) =>
    get<{ success: boolean; grants: any[] } & ServiceFallback>(`/api/doctor/grants/${encodeURIComponent(wallet)}`);

export const getDoctorAppointments = (wallet: string) =>
    get<{ success: boolean; appointments: any[] } & ServiceFallback>(`/api/doctor/appointments/${encodeURIComponent(wallet)}`);
...
export const getConsultationNotes = (doctorWallet: string, patientWallet: string) =>
    get<{ success: boolean; notes: any[] } & ServiceFallback>(`/api/doctor/notes/${encodeURIComponent(doctorWallet)}/${encodeURIComponent(patientWallet)}`);
...
export const getAccessGrants = (analysisId: string) =>
    get<{ success: boolean; grants: any[] } & ServiceFallback>(`/api/access-grants/analysis/${encodeURIComponent(analysisId)}`);
